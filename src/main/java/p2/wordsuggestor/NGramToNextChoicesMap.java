...
import process.misc.LargeValueFirstItemComparator;
...
public class NGramToNextChoicesMap {
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        ...
        if (k < 0) {
            ...
            Item<String, Integer>[] temp = new Item[afterNGrams.length];
            for(int i = 0; i < Math.min(k, afterNGrams.length); i++){
                temp[i] =  afterNGrams[Math.min(k, afterNGrams.length) - i - 1];
            }
            afterNGrams = temp;
        }
